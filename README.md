...
- **Git identity** — `user.name` / `user.email` are read from the host's global git config (values, not the file, so includes resolve and host-only settings like credential helpers are not carried over), staged as `.devcontainer/host-gituser`, and applied in the container via `git config --global` on every start. Keys unset on the host are left alone.
